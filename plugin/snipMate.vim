...
"
"
" Add opens a buffer with the snippets for the current filetype
" This is made for learning purpose
" There are snippetswhere every single snippet is a file This gives an
" overview
"
fun!  SnipList()
    let l:bufname = "snippets_list"
    let result = ""  
    for scope in [bufnr('%')] + split(&ft, '\.') + ['_']
        let triggers = has_key(s:snippets, scope) ? keys(s:snippets[scope]) : []
        for trigger in sort(triggers)
            let result = result .  trigger . "------------------------------------------". "\n"
            let result = result . s:snippets[scope][trigger] . "\n\n"
        endfor
        if has_key(s:multi_snips, scope)
            let m_triggers = keys(s:multi_snips[scope])
            for m_trigger in m_triggers
                let result = result . "\n" . m_trigger . "-----------------------------------------m" . "\n"
                let m_t =  s:multi_snips[scope][m_trigger]
                for m in m_t
                    for n in m
                        let result = result .  n . "\n\n\n"
                    endfor
    ...
    execute 'vert new '. l:bufname
    setlocal buftype=nofile bufhidden=delete noswapfile filetype=txt
